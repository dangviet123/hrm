                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody class="trunpading">
                                            <tr v-for="(listfile, index) in DataFiles" :key="index">
                                                ...
                                                <td class="text-center"> 
                                                    <a v-if="DataFiles[index].idPromote>0" download="" :href="'./public/images/promote/'+DataFiles[index].FileNameBase64"><i class="fa fa-download"></i></a>
                                                </td>
                                                <td class="text-center"> 
...
export default {
    data() {
        return {
            ...
            idPromote: this.$route.params.idPromote,
        }
    },
    ...
    methods: {
        async getInfo(idPromote){
			const self =this;
            self.$Progress.start();
            
            const res = await axios.get("/staff-information/promote/"+idPromote+"/edit");
        ...
        async updateList(){
			...
			try{
				...
				if (!self.$v.$invalid) {
					...
					self.isSave = true;
					self.$Progress.start();
					const res = await axios.put("/staff-information/promote/update",{listGroup: self.listGroup,DataFiles: self.DataFiles});
					if (res.data.Status == true) {
						self.$Progress.finish();
            ...
            if (self.listGroup.Locked == 0) {
                if (self.DataFiles.length > 1) {
                    self.$swal({
                        ...
                        if (result.value) {
                            if (self.DataFiles[index].idPromote > 0) {
                                axios.post("/staff-information/promote/"+self.DataFiles[index].idPromote+"/"+self.DataFiles[index].idPromoteFile+"/deleteListFile",{FileNameBase64: self.DataFiles[index].FileNameBase64})
                                .then(res => {
                                    if (res.data.Status == true) {
                                        self.$notify({
                                            group: 'infomation',
