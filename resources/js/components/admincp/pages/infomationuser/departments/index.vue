		...
		<div class="col-md-12 col-sm-12" v-if="isLoadingPage">
			...
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    ...
			    <li class="breadcrumb-item"><a href="#">Thôn tin nhân viên</a></li>
			...
			<div class="x_panel isFilter" v-show="isFilter" >
				...
				<div class="x_content" >
					<div class="col-md-4">
						...
						<input type="text"  v-model="filedSearch.DepartmentName" class="form-control" >
					</div>
					<div class="col-md-4">
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idDepartment="idDepartment" ></component>
			      </div>
			    </div>
			  </div>
			</div>
                ...
                <table class="table table-hover table-bordered">
					<thead>
						<tr class="headings">
							...
							<th @click="orderByColum('DepartmentName')">Tên phòng
								<span class="icon_sort" v-html="showIconOrderBy('DepartmentName',this.orderby)"></span>
							</th>
						...
						<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
							...
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
									<i @click="activeStatusOne(data.idDepartment,1)" v-tooltip.top-center="'Hoạt động'" class="fa fa-check text text-success" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
									<i @click="activeStatusOne(data.idDepartment,0)" v-tooltip.top-center="'Tạm ngưng'" class="fa fa-close text text-danger" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1">
	...
	export default {
	    ...
	    methods: {
			...
			update(){
				...
				for (let i = 0; i < self.datas.length; i++) 
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idDepartment);
					}
				}
				if (data_active.length == 1) {
					let idDepartment = data_active[0];
					$('#exampleModal').modal('show');
			...
			showIndex(){
				...
				self.isLoading = true;
				axios.get("/staff-information/departments",{params: self.$route.query})
				.then(res => {
			...
			activeStatusOne(idDepartment,Active){
				const self = this;
				self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/staff-information/departments/activeStatusOne",{idDepartment: idDepartment,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.post("/staff-information/departments/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.delete("/staff-information/departments/destroy",{params: {'data_active': data_active}})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idDepartment.push(self.datas[i].idDepartment);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/staff-information/departments/sortBy",{idDepartment: idDepartment,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/staff-information/departments/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["PHÒNG BAN"];
                var columcel = ["STT","Mã","Tên phòng","Mô Tả","Mã người lập","Người lập","Ngày lập","Tình trạng"];
				
				var dateTime = new Date();
				var filename = "departments-"+moment(dateTime).format("DD-MM-YYYY-HH-mm-ss")+".xlsx";
