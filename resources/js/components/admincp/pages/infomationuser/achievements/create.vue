...
export default {
    ...
    methods: {
        async getInfo(){
			const self =this;
			self.$Progress.start();
			const res = await axios.get("/staff-information/achievements/create");
        ...
        async createList(){
			...
			try{
				...
				if (!self.$v.$invalid) {
					...
					self.isSave = true;
					self.$Progress.start();
					const res = await axios.post("/staff-information/achievements",{listGroup: self.listGroup,DataFiles: self.DataFiles});
					if (res.data.Status == true) {
						self.$Progress.finish();
