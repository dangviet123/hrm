				...
				<div class="x_panel isFilter" v-show="isFilter" >
					...
					<div class="x_content" >
						...
						<div class="col-md-4">
							...
							<input type="text"  v-model="filedSearch.CompanyName" class="form-control" >
						</div>
						<div class="col-md-4">
							<label >Mô tả</label>
                    ...
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body content-fix">
                                <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idCompany="idCompany" ></component>
                            </div>
                        </div>
                    </div>
                </div>
					...
					<table class="table table-hover table-bordered">
						<thead>
							<tr class="headings">
								...
								<th @click="orderByColum('CompanyName')" >Tên công ty
									<span class="icon_sort" v-html="showIconOrderBy('CompanyName',this.orderby)"></span>
								</th>
								<th @click="orderByColum('Address')" >Địa chỉ
									<span class="icon_sort" v-html="showIconOrderBy('Address',this.orderby)"></span>
								</th>
							...
							<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
								...
								<td class="text-center" @click="datas[index].checked=!datas[index].checked">
									<v-popover  placement="right">
										<span class="tooltip-target b3">{{ data.IDCode }}</span>
										<template slot="popover">
											<div class="popover-body">
												<div><b class="title-infobb">Mã số thuế:</b> {{data.TaxCode}}</div>
												<div><b class="title-infobb">Điện thoại:</b> {{data.Phone}}</div>
												<div><b class="title-infobb">Giám đốc:</b> {{data.Director}}</div>
												<div><b class="title-infobb">Quản lý:</b> {{data.Manager}}</div>
												<div><b class="title-infobb">Tài khoản ngân hàng:</b> {{data.BankAccount}}</div>
												<div><b class="title-infobb">Mô tả:</b> {{data.Description}}</div>
												<div><b class="title-infobb">Ghi chú:</b> {{data.Notes}}</div>
												<div><b class="title-infobb">Người lập:</b> {{data.FullName}}</div>
												<div><b class="title-infobb">Ngày lập:</b> {{data.created_at | fomatDate}}</div>
											</div>
										</template>
									</v-popover>
								</td>
								...
								<td class="text-center" @click="datas[index].checked=!datas[index].checked">
									<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
										<i @click="activeStatusOne(data.idCompany,1)" v-tooltip.top-center="'Hoạt động'" class="fa fa-check text text-success" ></i>
									</a>
									<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
										<i @click="activeStatusOne(data.idCompany,0)" v-tooltip.top-center="'Tạm ngưng'" class="fa fa-close text text-danger" ></i>
									</a>
									<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1">
	...
	export default {
	    ...
	    methods: {
			...
			update(){
				...
				for (let i = 0; i < self.datas.length; i++) 
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idCompany);
					}
				}
				if (data_active.length == 1) {
					let idCompany = data_active[0];
					$('#myLargeModal').modal('show');
			...
			showIndex(){
				...
				self.isLoading = true;
				axios.get("/staff-information/companys",{params: self.$route.query})
				.then(res => {
			...
			activeStatusOne(idCompany,Active){
				const self = this;
				self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/staff-information/companys/activeStatusOne",{idCompany: idCompany,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.post("/staff-information/companys/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.delete("/staff-information/companys/destroy",{params: {'data_active': data_active}})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idCompany.push(self.datas[i].idCompany);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/staff-information/companys/sortBy",{idCompany: idCompany,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/staff-information/companys/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["CÔNG TY"];
                var columcel = ["STT","Mã","Tên công ty","Địa chỉ","Điện thoại","Mã số thuế","Giám đốc","Quản lý","Tài khoản ngân hàng","Mô tả","Ghi chú","Mã người lập","Người lập","Ngày lập","Tình trạng"];
				
				var dateTime = new Date();
				var filename = "companys-"+moment(dateTime).format("DD-MM-YYYY-HH-mm-ss")+".xlsx";
                var sheetname = "companys";
                ...
                data.forEach(el => {
                    ...
                    createXLSLFormatObj.push([
						...
						el.TaxCode==null ? '' : el.TaxCode,
						el.Director==null ? '' : el.Director,
						el.Manager==null ? '' : el.Manager,
						el.BankAccount==null ? '' : el.BankAccount,
