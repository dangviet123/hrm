...
export default {
    props: ['idCompany'],
	watch: { 
		idCompany(newVal, oldVal) { // watch it
			this.getInfo(newVal);
		}
    },
    data() {
            ...
            IDCode: {
                ...
                isUnique(value){
                    ...
                    return new Promise((resolve, reject) => {
                        axios.post("/staff-information/companys/checkIDCode",{IDCode: value,idCompany: this.idCompany})
                        .then(res => {
    ...
    created(){
		const self =this;
        self.getInfo(self.idCompany);
	},
    methods: {
        getInfo(idCompany){
            const self =this;
            self.$Progress.start();
			axios.get("/staff-information/companys/"+idCompany+"/edit")
			.then(res => {
        ...
        updateList(){
            ...
            if (!self.$v.$invalid) {
				...
				self.isSave = true;
				self.$Progress.start();
                axios.put("/staff-information/companys/update",{listGroup: self.listGroup})
				.then(res => {
					if (res.data.Status == true) {
						self.$Progress.finish();
