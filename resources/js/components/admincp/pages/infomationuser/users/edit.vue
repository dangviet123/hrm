					...
					<div class="x_content">
						<div class="col-md-12">
							...
							<div class="profile_img" style="text-align: right;" v-if="listGroup.Image !=''">
								<div id="crop-avatar">
									<img class="img-responsive avatar-view" :src="(listGroup.Image == listGroup.ImageOl) ? './public/images/users/'+listGroup.Image : listGroup.Image " style="width: 200px;height: 200px;cursor: pointer;">
									<div class="delete-avt">
										...
										<label class=" btn btn-danger btn-sm" @click="deleteImage" v-tooltip.top-center="'Xóa'"><i class="fa fa-trash-o"></i></label>
									</div>
								</div>
								<br>
							</div>
							...
							<div class="profile_img" style="text-align: right;" v-if="listGroup.Image ==''">
								<div id="crop-avatar">
									...
									<div class="delete-avt">
										...
										<label class=" btn btn-danger btn-sm" v-tooltip.top-center="'Xóa'" @click="deleteImage"><i class="fa fa-trash-o"></i></label>
									</div>
								</div>
								<br>
							</div>
							...
							<table style="width:100%">
								<tr>
									...
									<td style="text-align: right;">
										<p-check class="p-switch p-fill" color="success_gren" v-tooltip.top-center="listGroup.Active==1 ? 'Hoạt động': 'Tạm ngưng'" :checked="listGroup.Active" v-model="listGroup.Active" >
								...
								<tr>
									...
									<td style="text-align: right;">
										<p-check class="p-switch p-fill" color="info" v-tooltip.top-center="listGroup.isWork==1 ? 'Còn làm': 'Đã nghỉ'" :checked="listGroup.isWork" v-model="listGroup.isWork" >
						...
						<table class="table table-bordered">
							<tr>
								...
								<th class="text-center" style="width:5%">
									<a class="badge" v-tooltip.top-center="'Thêm'">
										<i @click="addRowFiles" class="fa fa-plus-square" style="font-size: 120%;"></i>
									</a>
								</th>
							</tr>
							<tbody class="trunpading">
								<tr v-for="(listfile, index) in DataFiles" :key="index">
									...
									<td class="text-center"> 
										<a v-tooltip.top-center="'Tập tin'" v-if="DataFiles[index].idUser>0" download="" :href="'./public/images/user_files/'+DataFiles[index].FileNameBase64"><i class="fa fa-download"></i></a>
									</td>
									<td class="text-center"> 
										{{ DataFiles[index].Size | prettyBytes}}
									</td>
									<td class="text-center">
										<a class="badge" v-tooltip.top-center="'Xóa'">
											<i @click="removeRowFiles(index)" class="fa fa-close text-danger"></i>
										</a>
									</td>
								</tr>
							</tbody>
...
export default {
		...
		listGroup: {
			...
			Email: {required,email,minLength: minLength(1),maxLength: maxLength(225),
				async isUnique(value){
                    ...
                    const response = await axios.post("/staff-information/users/checkEmail",{Email: value,idUser: this.idUser});
            ...
            IDCode: {
                ...
                async isUnique(value){
                    if (value == '') return true
                    const response = await axios.post("/staff-information/users/checkIDCode",{IDCode: value,idUser: this.idUser});
                    return response.data.Status;
                },
	...
	methods: {
        async getInfo(idUser){
			...
			self.isSave = true;
            self.$Progress.start();
			const res = await axios.get("/staff-information/users/"+idUser+"/edit");
			if (res.data.Status == true) {
				...
				if (self.Permission.indexOf(4) ==-1) {self.$router.push({name: 'dashboard'});}
			}
		...
		removeRowFiles(index){
			...
			if (self.DataFiles.length > 1) {
				...
				.then(function(dialog) {
					if (self.DataFiles[index].idUser > 0) {
						axios.post("/staff-information/users/"+self.DataFiles[index].idUser+"/"+self.DataFiles[index].idFile+"/deleteListFile",{FileNameBase64: self.DataFiles[index].FileNameBase64})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.$notify({
		...
		deleteImage(){
			const self = this;
			self.$dialog.confirm({title: "THÔNG BÁO?",body: "ạn muốn xóa ảnh ?"})
			.then(function(dialog) {
				 axios.post("/staff-information/users/"+self.idUser+"/deleteImage",{Image: self.listGroup.Image})
				.then(res => {
					if (res.data.Status == true) {
						dialog.close();
						self.$notify({
		...
		async updateList(){
			...
			try{
				...
				if (!self.$v.$invalid) {
					...
					self.isSave = true;
					self.$Progress.start();
					const res = await axios.put("/staff-information/users/update",{listGroup: self.listGroup,DataFiles: self.DataFiles});
					if (res.data.Status == true) {
						self.$Progress.finish();
						self.isDisable = false;
						self.isSave = false;
