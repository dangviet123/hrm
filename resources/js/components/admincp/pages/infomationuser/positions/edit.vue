<template>
    <div>
        ...
        <div class="col-md-12">
            <div class="formform-group" :class="{ 'form-group--error':$v.listGroup.PositionName.$error }">
                <label>Chức vụ<span class="text-danger" >*</span></label>
                <input type="text"
                v-model="$v.listGroup.PositionName.$model"
                :class="{'border border-danger': $v.listGroup.PositionName.$error}" 
                :disabled="isDisable" 
                class="form-control">
                <span v-if="!$v.listGroup.PositionName.required" class="text-danger error">Trường không được để trống</span>
                <span v-if="!$v.listGroup.PositionName.minLength" class="text-danger error">Vui lòng nhập tối thiểu {{$v.listGroup.PositionName.$params.minLength.min}} kí tự</span>
                <span v-if="!$v.listGroup.PositionName.maxLength" class="text-danger error">Vui lòng nhập tối da {{$v.listGroup.PositionName.$params.maxLength.max}} kí tự</span>
            </div>
...
export default {
    props: ['idPosition'],
	watch: { 
		idPosition	(newVal, oldVal) { // watch it
			this.getInfo(newVal);
		}
    },
    data() {
    ...
    methods: {
        ...
        getInfo(idPosition){ // lấy thông tin
			const self =this;
			self.$Progress.start();
			axios.get("/staff-information/positions/"+idPosition+"/edit")
			.then(res => {
        ...
        updateList(){
            ...
            if (!self.$v.$invalid) {
				...
				self.isSave = true;
                self.$Progress.start();
                axios.put("/staff-information/positions/update",{listGroup: self.listGroup})
				.then(res => {
					if (res.data.Status == true) {
