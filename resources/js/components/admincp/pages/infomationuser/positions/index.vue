		...
		<div class="col-md-12 col-sm-12" v-if="isLoadingPage">
			...
			<div class="x_panel isFilter" v-show="isFilter" >
				...
				<div class="x_content" >
					<div class="col-md-4">
						<label>Chức vụ</label>
						<input type="text"  v-model="filedSearch.PositionName" class="form-control" >
					</div>
					<div class="col-md-4">
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idPosition="idPosition" ></component>
			      </div>
			    </div>
			  </div>
			</div>
                ...
                <table class="table table-hover table-bordered">
					<thead>
						<tr class="headings">
							...
							<th @click="orderByColum('PositionName')">Chức vụ
								<span class="icon_sort" v-html="showIconOrderBy('PositionName',this.orderby)"></span>
							</th>
						...
						<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
							...
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
									<i @click="activeStatusOne(data.idPosition,1)" v-tooltip.top-center="'Hoạt động'" class="fa fa-check text text-success" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
									<i @click="activeStatusOne(data.idPosition,0)" v-tooltip.top-center="'Tạm ngưng'" class="fa fa-close text text-danger" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1">
	...
	export default {
	    ...
	    methods: {
			...
			update(){
				...
				for (let i = 0; i < self.datas.length; i++) 
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idPosition);
					}
				}
				if (data_active.length == 1) {
					let idPosition = data_active[0];
					$('#exampleModal').modal('show');
			...
			showIndex(){
				...
				self.isLoading = true;
				axios.get("/staff-information/positions",{params: self.$route.query})
				.then(res => {
			...
			activeStatusOne(idPosition,Active){
				const self = this;
				self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/staff-information/positions/activeStatusOne",{idPosition: idPosition,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.post("/staff-information/positions/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.delete("/staff-information/positions/destroy",{params: {'data_active': data_active}})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idPosition.push(self.datas[i].idPosition);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/staff-information/positions/sortBy",{idPosition: idPosition,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/staff-information/positions/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["CHỨC VỤ"];
                var columcel = ["STT","Mã","Chức vụ","Mô Tả","Mã người lập","Người lập","Ngày lập","Tình trạng"];
				
				var dateTime = new Date();
				var filename = "positions-"+moment(dateTime).format("DD-MM-YYYY-HH-mm-ss")+".xlsx";
