			...
			<div class="x_panel isFilter" v-show="isFilter" >
				...
				<div class="x_content" >
					<div class="col-md-4">
						...
						<input type="text"  v-model="filedSearch.LevelName" class="form-control" >
					</div>
					<div class="col-md-4">
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idLevel="idLevel" ></component>
			      </div>
			    </div>
			  </div>
			</div>
                ...
                <table class="table table-hover table-bordered">
					<thead>
						<tr class="headings">
							...
							<th @click="orderByColum('LevelName')" >Chức vụ
								<span class="icon_sort" v-html="showIconOrderBy('LevelName',this.orderby)"></span>
							</th>
						...
						<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
							...
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
									<i @click="activeStatusOne(data.idLevel,1)" v-tooltip.top-center="'Hoạt động'" class="fa fa-check text text-success" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
									<i @click="activeStatusOne(data.idLevel,0)" v-tooltip.top-center="'Tạm ngưng'" class="fa fa-close text text-danger" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1">
	...
	export default {
	    ...
	    methods: {
			...
			update(){
				...
				for (let i = 0; i < self.datas.length; i++) 
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idLevel);
					}
				}
				if (data_active.length == 1) {
					let idLevel = data_active[0];
					$('#exampleModal').modal('show');
			...
			showIndex(){
				...
				self.isLoading = true;
				axios.get("/staff-information/levels",{params: self.$route.query})
				.then(res => {
			...
			activeStatusOne(idLevel,Active){
				const self = this;
				self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/staff-information/levels/activeStatusOne",{idLevel: idLevel,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.post("/staff-information/levels/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.delete("/staff-information/levels/destroy",{params: {'data_active': data_active}})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idLevel.push(self.datas[i].idLevel);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/staff-information/levels/sortBy",{idLevel: idLevel,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/staff-information/levels/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["TRÌNH ĐỘ"];
                var columcel = ["STT","Mã","Trình độ","Mô Tả","Mã người lập","Người lập","Ngày lập","Tình trạng"];
				
				var dateTime = new Date();
				var filename = "levels-"+moment(dateTime).format("DD-MM-YYYY-HH-mm-ss")+".xlsx";
                var sheetname = "levels";
