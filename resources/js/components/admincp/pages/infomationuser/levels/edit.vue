...
export default {
    props: ['idLevel'],
	watch: { 
		idLevel	(newVal, oldVal) { // watch it
			this.getInfo(newVal);
		}
    },
    data() {
    ...
    methods: {
        ...
        getInfo(idLevel){ // lấy thông tin
			const self =this;
			self.$Progress.start();
			axios.get("/staff-information/levels/"+idLevel+"/edit")
			.then(res => {
        ...
        updateList(){
            ...
            if (!self.$v.$invalid) {
				...
				self.isSave = true;
                self.$Progress.start();
                axios.put("/staff-information/levels/update",{listGroup: self.listGroup})
				.then(res => {
					if (res.data.Status == true) {
