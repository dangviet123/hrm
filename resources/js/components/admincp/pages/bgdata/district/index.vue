			...
			<div class="x_panel isFilter" v-show="isFilter" >
				<div class="x_content" >
					...
					<div class="col-md-4">
						...
						<input type="text"  v-model="filedSearch.District" class="form-control" >
					</div>
					<div class="col-md-4">
						<label>Tỉnh/TP</label>
						<v-select taggable multiple :options="Provinces" label="Provinces" :reduce="tag => tag.idProvinces" v-model="filedSearch.idProvinces" ></v-select>
					</div>
					<div class="col-md-4">
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idDistrict="idDistrict" ></component>
			      </div>
			    </div>
			  </div>
			</div>
                ...
                <table class="table table-hover table-bordered">
					<thead>
						<tr class="headings">
							...
							<th @click="orderByColum('District')">Quận huyện
								<span class="icon_sort" v-html="showIconOrderBy('District',this.orderby)"></span>
							</th>
						...
						<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
							...
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
									<i @click="activeStatusOne(data.idDistrict,1)" v-tooltip.top-center="'Hoạt động'" class="fa fa-check text text-success" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
									<i @click="activeStatusOne(data.idDistrict,0)" v-tooltip.top-center="'Tạm ngưng'" class="fa fa-close text text-danger" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1">
	...
	export default {
	    ...
	    methods: {
			...
			update(){
				...
				for (let i = 0; i < self.datas.length; i++) 
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idDistrict);
					}
				}
				if (data_active.length == 1) {
					let idDistrict = data_active[0];
					$('#exampleModal').modal('show');
			...
			showIndex(){
				...
				self.isLoading = true;
				axios.get("/bgdata/district",{params: self.$route.query})
				.then(res => {
			...
			activeStatusOne(idDistrict,Active){
				const self = this;
				self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/bgdata/district/activeStatusOne",{idDistrict: idDistrict,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.post("/bgdata/district/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.delete("/bgdata/district/destroy",{params: {'data_active': data_active}})
							.then(res => {
								if (res.data.Status == true) {
									dialog.close();
									self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idDistrict.push(self.datas[i].idDistrict);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/bgdata/district/sortBy",{idDistrict: idDistrict,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/bgdata/district/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["QUẬN HUYỆN"];
                var columcel = ["STT","Mã quận huyện","Quận huyện","Mã Tỉnh/TP","Tỉnh/TP","Ghi chú","Mã người lập","Người lập","Ngày lập","Tình trạng"];
				
				var dateTime = new Date();
				var filename = "district-"+moment(dateTime).format("DD-MM-YYYY-HH-mm-ss")+".xlsx";
                var sheetname = "district";
