...
export default {
    props: ['idDistrict'],
	watch: { 
		idDistrict	(newVal, oldVal) { // watch it
			this.getInfo(newVal);
		}
    },
    data() {
    ...
    methods: {
        ...
        getInfo(idDistrict){ // lấy thông tin
			const self =this;
			self.$Progress.start();
			axios.get("/bgdata/district/"+idDistrict+"/edit")
			.then(res => {
        ...
        updateList(){
            ...
            if (!self.$v.$invalid) {
				...
				self.isSave = true;
                self.$Progress.start();
                axios.put("/bgdata/district/update",{listGroup: self.listGroup})
				.then(res => {
					if (res.data.Status == true) {
