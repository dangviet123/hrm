<template>
    <div>
        ...
        <div class="col-md-12">
            <div class="formform-group" :class="{ 'form-group--error':$v.listGroup.GroupName.$error }">
                <label>Tên Nhóm<span class="text-danger" >*</span></label>
                <input type="text"
                v-model="$v.listGroup.GroupName.$model"
                :class="{'border border-danger': $v.listGroup.GroupName.$error}" 
                :disabled="isDisable" 
                class="form-control">
                <span v-if="!$v.listGroup.GroupName.required" class="text-danger error">Trường không được để trống</span>
                <span v-if="!$v.listGroup.GroupName.minLength" class="text-danger error">Vui lòng nhập tối thiểu {{$v.listGroup.GroupName.$params.minLength.min}} kí tự</span>
                <span v-if="!$v.listGroup.GroupName.maxLength" class="text-danger error">Vui lòng nhập tối da {{$v.listGroup.GroupName.$params.maxLength.max}} kí tự</span>
            </div>
...
export default {
    props: ['idGroup'],
	watch: { 
		idGroup	(newVal, oldVal) { // watch it
			this.getInfo(newVal);
		}
    },
    data() {
    ...
    methods: {
        ...
        getInfo(idGroup){ // lấy thông tin
			const self =this;
			self.$Progress.start();
			axios.get("/bgdata/grouppermission/"+idGroup+"/edit")
			.then(res => {
        ...
        updateList(){
            ...
            if (!self.$v.$invalid) {
				...
				self.isSave = true;
                self.$Progress.start();
                axios.put("/bgdata/grouppermission/update",{listGroup: self.listGroup})
				.then(res => {
					if (res.data.Status == true) {
