		...
		<div class="col-md-12 col-sm-12" v-if="isLoadingPage">
			...
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    ...
			    <li class="breadcrumb-item active" aria-current="page">Phường xã</li>
			  </ol>
			</nav>
			...
			<div class="x_panel isFilter" v-show="isFilter" >
				...
				<div class="x_content" >
					...
					<div class="col-md-4">
						...
						<input type="text"  v-model="filedSearch.Ward" class="form-control" >
					</div>
					<div class="col-md-4">
						<label>Tỉnh/TP</label>
						<v-select  @input="loadDistrict"  taggable multiple :options="Provinces" label="Provinces" :reduce="tag => tag.idProvinces" v-model="filedSearch.idProvinces" ></v-select>
					</div>
					<div class="col-md-4">
						<label>Quận huyện</label>
						<v-select taggable multiple :options="District" label="District" :reduce="tag => tag.idDistrict" v-model="filedSearch.idDistrict" ></v-select>
					</div>
					<div class="col-md-4">
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idWard="idWard" ></component>
			      </div>
			    </div>
			  </div>
			</div>
                ...
                <table class="table table-hover table-bordered">
					<thead>
						<tr class="headings">
							...
							<th @click="orderByColum('Ward')">Phường xã
								<span class="icon_sort" v-html="showIconOrderBy('Ward',this.orderby)"></span>
							</th>
							<th @click="orderByColum('idDistrict')">Quận huyện
								<span class="icon_sort" v-html="showIconOrderBy('idDistrict',this.orderby)"></span>
							</th>
							<th @click="orderByColum('idProvinces')">Tỉnh/TP
								<span class="icon_sort" v-html="showIconOrderBy('idProvinces',this.orderby)"></span>
							</th>
					...
					<draggable v-model="datas" tag="tbody" handle=".handle"  @change="sortBy" >
						<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
							...
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a class="badge" v-tooltip.top-center="'Hoạt động'" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
									<i @click="activeStatusOne(data.idWard,1)"  class="fa fa-check text text-success" ></i>
								</a>
								<a class="badge" v-tooltip.top-center="'Tạm ngưng'" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
									<i @click="activeStatusOne(data.idWard,0)"  class="fa fa-close text text-danger" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1" v-tooltip.top-center="data.Active==1 ? 'Hoạt động' : 'Tạm ngưng'">
									<i  :class="data.Active==1 ? 'fa fa-check' : 'fa fa-close'" ></i>
								</a>
								<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) == -1" v-tooltip.top-center="data.Active==1 ? 'Hoạt động' : 'Tạm ngưng'">
									<i  :class="data.Active==1 ? 'fa fa-check' : 'fa fa-close'" ></i>
								</a>
							</td>
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a href="javascript:" class="badge"  v-tooltip.top-center="'Sắp xếp'"><i class="fa fa-arrows handle" ></i></a>
							</td>
						</tr>
					</draggable>
...
<script>
	...
	export default {
		...
		data () {
	      return {
			...
			filedSearch: {
				...
				idProvinces: (this.$route.query.idProvinces) ? this.$route.query.idProvinces.split(',') : [],
				idDistrict: (this.$route.query.idDistrict) ? this.$route.query.idDistrict.split(',') : []
			},
			pagination: {},
			limit: 0,
		...
		beforeRouteEnter(to, from, next) {
			try {
				next(self => {
					if (self.filedSearch.idDistrict.length > 0) {
						for (let i = 0; i < self.filedSearch.idDistrict.length; i++) 
						self.filedSearch.idDistrict[i] = parseInt(self.filedSearch.idDistrict[i]);
					}
					if (self.filedSearch.idProvinces.length > 0) {
						for (let i = 0; i < self.filedSearch.idProvinces.length; i++) 
						self.filedSearch.idProvinces[i] = parseInt(self.filedSearch.idProvinces[i]);
					}
					self.showIndex();
				})
			} catch (err) {
	    ...
	    methods: {
			loadDistrict(){
				const self = this;
				self.$Progress.start();
				axios.post("/bgdata/ward/loadDistrict",{idProvinces: self.filedSearch.idProvinces})
				.then(res => {
					self.District = res.data.District;
					self.filedSearch.idDistrict = [];
			...
			update(){
				...
				for (let i = 0; i < self.datas.length; i++) 
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idWard);
					}
				}
				if (data_active.length == 1) {
					let idWard = data_active[0];
					$('#exampleModal').modal('show');
			...
			async showIndex(){
				...
				self.isLoading = true;
				const res = await axios.get("/bgdata/ward",{params: self.$route.query});
			...
			activeStatusOne(idWard,Active){
				const self = this;
				self.$dialog
				.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/bgdata/ward/activeStatusOne",{idWard: idWard,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.post("/bgdata/ward/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.delete("/bgdata/ward/destroy",{params: {'data_active': data_active}})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idWard.push(self.datas[i].idWard);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/bgdata/ward/sortBy",{idWard: idWard,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/bgdata/ward/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["PHƯỜNG XÃ"];
                var columcel = ["STT","Mã","Phường xã","Mã quận huyện","Quận huyện","Mã Tỉnh/TP","Tỉnh/TP","Ghi chú","Mã người lập","Người lập","Ngày lập","Tình trạng"];
				var dateTime = new Date();
				var filename = "ward-"+moment(dateTime).format("DD-MM-YYYY-HH-mm-ss")+".xlsx";
                var sheetname = "ward";
