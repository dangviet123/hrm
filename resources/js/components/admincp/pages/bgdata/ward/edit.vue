...
export default {
    ...
    methods: {
        ...
        getInfo(idWard){ // lấy thông tin
			const self =this;
			self.$Progress.start();
			axios.get("/bgdata/ward/"+idWard+"/edit")
			.then(res => {
        ...
        updateList(){
            ...
            if (!self.$v.$invalid) {
				...
				self.isSave = true;
                self.$Progress.start();
                axios.put("/bgdata/ward/update",{listGroup: self.listGroup})
				.then(res => {
					if (res.data.Status == true) {
