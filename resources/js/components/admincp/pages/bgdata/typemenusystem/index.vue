		...
		<div class="col-md-12 col-sm-12" v-if="isLoadingPage" >
			...
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    ...
			    <li class="breadcrumb-item active" aria-current="page">Loại menu</li>
			  </ol>
			</nav>
			<div class="x_content_button" >
				...
				<a v-tooltip.top-center="DataInfoPermission[9].ListName" v-if="Permission.indexOf(9) !=-1" class="btn btn-defaults" @click="createdNewListMenu" >
					<i :class="DataInfoPermission[9].Icon"></i>
				</a>
			...
			<div class="x_panel isFilter" v-show="isFilter" >
				...
				<div class="x_content" >
					<div class="col-md-4">
						<label for="TypeMenuName">Tên loại</label>
						<input type="text" id="TypeMenuName" v-model="filedSearch.TypeMenuName" class="form-control" >
					</div>
					
					<div class="col-md-4">
						<label for="Description">Mô tả</label>
						<input type="text" id="Description" v-model="filedSearch.Description" class="form-control" >
					</div>
					<div class="col-md-4">
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idTypeMenu="idTypeMenu" ></component>
			      </div>
			    </div>
			  </div>
			</div>
                ...
                <table class="table table-hover table-bordered">
					<thead>
						<tr class="headings">
							...
							<th @click="orderByColum('TypeMenuName')" >Tên loại
								<span class="icon_sort" v-html="showIconOrderBy('TypeMenuName',this.orderby)"></span>
							</th>
							<th @click="orderByColum('Description')" >Mô tả
								<span class="icon_sort" v-html="showIconOrderBy('Description',this.orderby)"></span>
							</th>
							<th @click="orderByColum('idUserAdd')" style="width:150px" class="text-center">Người lập
						...
						<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
							...
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
									<i @click="activeStatusOne(data.idTypeMenu,1)" v-tooltip.top-center="'Hoạt động'" class="fa fa-check text text-success" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
									<i @click="activeStatusOne(data.idTypeMenu,0)" v-tooltip.top-center="'Tạm ngưng'" class="fa fa-close text text-danger" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1">
	...
	export default {
		...
		data () {
	      return {
			...
			filedSearch: {
				...
				TypeMenuName: this.$route.query.TypeMenuName,
	    ...
	    methods: {
			...
			createdNewListMenu(){
				...
				for (let i = 0; i < self.datas.length; i++) {
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idTypeMenu);
						
					}
				}
				if(data_active.length == 1) {
					let idTypeMenu = data_active[0];
					self.$router.push({path:'/bgdata/typemenusystem/'+idTypeMenu+'/listmenu'});
				}else{
					self.$notify({
						group: 'infomation',
						type: 'error',
			...
			showIndex(){
				...
				self.isLoading = true;
				axios.get("/bgdata/typemenusystem",{params: self.$route.query})
				.then(res => {
			...
			activeStatusOne(idTypeMenu,Active){
				const self = this;
				self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/bgdata/typemenusystem/activeStatusOne",{idTypeMenu: idTypeMenu,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.post("/bgdata/typemenusystem/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.delete("/bgdata/typemenusystem/destroy",{params: {'data_active': data_active}})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idTypeMenu.push(self.datas[i].idTypeMenu);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/bgdata/typemenusystem/sortBy",{idTypeMenu: idTypeMenu,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/bgdata/typemenusystem/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["LOẠI MENU"];
                var columcel = ["STT","Tên loại","Mô tả","Mã người lập","Người lập","Ngày lập","Tình trạng"];
				var dateTime = new Date();
				var filename = "listpermission-"+moment(dateTime).format("DD-MM-YYYY-HH-mm-ss")+".xlsx";
                var sheetname = "listpermission";
