		...
		<div class="col-md-12 col-sm-12" v-if="isLoadingPage">
			...
			<nav aria-label="breadcrumb">
			  <ol class="breadcrumb">
			    ...
			    <li class="breadcrumb-item active" aria-current="page">Danh mục quyền</li>
			  </ol>
			</nav>
			...
			<div class="x_panel isFilter" v-show="isFilter" >
				...
				<div class="x_content" >
					<div class="col-md-4">
						<label for="ListName">Tên quyền</label>
						<input type="text" id="ListName" v-model="filedSearch.ListName" class="form-control" >
					</div>
					<div class="col-md-4">
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <component v-bind:is="isComponent" @listenisSaveCheck="listenisSave" :idList="idList" ></component>
			      </div>
			    </div>
			  </div>
			</div>
                ...
                <table class="table table-hover table-bordered">
					<thead>
						<tr class="headings">
							...
							<th @click="orderByColum('ListName')">Tên quyền
								<span class="icon_sort" v-html="showIconOrderBy('ListName',this.orderby)"></span>
							</th>
							<th @click="orderByColum('Icon')" style="width:100px" class="text-center">Biểu tượng
								<span class="icon_sort" v-html="showIconOrderBy('Icon',this.orderby)"></span>
							</th>
					...
					<draggable v-model="datas" tag="tbody" handle=".handle"  @change="sortBy" >
						<tr v-for="(data, index) in datas" :key="index" :class="{'tr-active': data.checked}" @click="datas[index].checked=!datas[index].checked">
							...
							<td>{{ data.ListName }}</td>
							<td class="text-center"><a class="badge" href="javascript:"><i :class="data.Icon" ></i></a></td>
							...
							<td class="text-center" @click="datas[index].checked=!datas[index].checked">
								<a class="badge" v-if="data.Active==1 && Permission.indexOf(6) !=-1">
									<i @click="activeStatusOne(data.idList,1)" v-tooltip.top-center="'Hoạt động'" class="fa fa-check text text-success" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) !=-1">
									<i @click="activeStatusOne(data.idList,0)" v-tooltip.top-center="'Tạm ngưng'" class="fa fa-close text text-danger" ></i>
								</a>
								<a class="badge" v-if="data.Active==0 && Permission.indexOf(5) == -1">
	...
	export default {
	    ...
	    methods: {
			...
			update(){
				...
				for (let i = 0; i < self.datas.length; i++) 
					if (self.datas[i].checked == true) {
						data_active.push(self.datas[i].idList);
					}
				}
				if (data_active.length == 1) {
					let idList = data_active[0];
					$('#exampleModal').modal('show');
			...
			showIndex(){
				...
				self.isLoading = true;
				axios.get("/bgdata/listpermission",{params: self.$route.query})
				.then(res => {
			...
			activeStatusOne(idList,Active){
				const self = this;
				self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
				.then(function(dialog) {
					axios.post("/bgdata/listpermission/activeStatusOne",{idList: idList,Active: Active})
					.then(res => {
						if (res.data.Status == true) {
							dialog.close();
							self.showIndex();
			...
			activeStatusAll(Active){
				...
				if (data_active.length > 0) {
					self.$dialog.confirm({title: "THÔNG BÁO?",body: "Bạn có muốn xác nhận thực hiện"})
					.then(function(dialog) {
						axios.post("/bgdata/listpermission/activeStatusAll",{data_active: data_active,Active: Active})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			deleteAll(){
				...
				if (data_active.length > 0) {
					...
					.then(function(dialog) {
						axios.delete("/bgdata/listpermission/destroy",{params: {'data_active': data_active}})
						.then(res => {
							if (res.data.Status == true) {
								dialog.close();
								self.showIndex();
			...
			sortBy(){
				...
				var orderby = [];
				for (let i = 0; i < self.datas.length; i++) {
					idList.push(self.datas[i].idList);
					orderby.push(self.datas[i].OrderBy);
				}
				orderby.sort(function(a,b) {return (self.orderby == 'ASC') ? a - b : b - a;});
				self.isLoading = true;
				axios.post("/bgdata/listpermission/sortBy",{idList: idList,orderby: orderby})
				.then(res => {
					if (res.data.Status == true) {
						self.showIndex();
						self.$notify({
			...
			async exportExcel() { // xuất excel
                const self = this;
                self.isLoading = true;
                const res = await axios.get("/bgdata/listpermission/1/exportExcel",{params: self.$route.query});
                let data = res.data.datas;
                let createXLSLFormatObj = [];
                var TitleName = ["NHÓM QUYỀN"];
                var columcel = ["STT","Tên nhóm","Biểu tượng","Mô tả","Mã người lập","Người lập","Ngày lập","Tình trạng"];
