...
export default {
    ...
    methods: {
        ...
        getInfo(idList){ // lấy thông tin
			const self =this;
			self.$Progress.start();
			axios.get("/bgdata/listpermission/"+idList+"/edit")
			.then(res => {
				if (res.data.Status == true) {
					self.listper =res.data.datas;
        ...
        updateList(){
            ...
            if (!self.$v.$invalid) {
				...
				self.isSave = true;
                self.$Progress.start();
                axios.put("/bgdata/listpermission/update",{listper: self.listper})
				.then(res => {
					if (res.data.Status == true) {
