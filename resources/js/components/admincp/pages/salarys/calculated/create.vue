...
export default {
    ...
    methods: {
        async getInfo(){
            ...
            self.$Progress.start();
            self.isLoading = true;
			const res = await axios.get("/salarys/calculated/create");
			if (res.data.Status == true) {
                self.listGroup.IDCode  = res.data.IDCode;
                self.listGroup.MonthlySalary  = res.data.MonthlySalary;
        ...
        async createList(){
			...
			try{
				...
				if (!self.$v.$invalid) {
					...
					self.isSave = true;
                     self.isLoading = true;
					const res = await axios.post("/salarys/calculated",{listGroup: self.listGroup,DataSalaryDataA: self.DataSalaryDataA,DataSalaryDataB:self.DataSalaryDataB});
					if (res.data.Status == true) {
						self.$Progress.finish();
